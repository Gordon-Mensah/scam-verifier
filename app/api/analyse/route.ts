...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!inputText || inputText.trim().length < 20) {
      return NextResponse.json(
        { error: 'Please provide more detail about the opportunity.' },
        { status: 400 }
      )
    }

    if (
      !questions?.amountAsked ||
      !questions?.promisedReturn ||
      !questions?.timePressure ||
      !questions?.howYouFound
    ) {
      return NextResponse.json(
    ...
    // Create report record — store clerk userId for future dashboard
